...
CREATE PROCEDURE add_member (
    ...
    IN p_contact_no BIGINT,
    IN p_joining_date DATE
)
BEGIN
    INSERT INTO members_name (member_id, name, email, contact_no, joining_date)
    VALUES (p_member_id, p_name, p_email, p_contact_no, p_joining_date);

    SELECT CONCAT('✅ Member ', p_name, ' has been added.') AS confirmation;
END$$

DELIMITER ;

CALL add_member(
    5,                        -- member_id (must be unique)
    'Radha Krishna',          -- name
    'radha@example.com',      -- email
    9876543210,               -- contact_no
    '2024-06-26'              -- joining_date
);
...
SELECT borrow_count(1) AS total_borrows;